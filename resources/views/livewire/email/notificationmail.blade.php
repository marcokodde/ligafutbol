...
<html lang="en">
    ...
    <body>
        ...
        <h1 class="text-blue-700">{{__('Notifications to Galveston Cup!')}}</h1>
            @if ($noty == "Noty Create User")
                @include('livewire.email.notify_create_user')
            @elseif($noty == "Create Payment")
                @include('livewire.email.notify_create_payment')
            @else
                @include('livewire.email.notify_error_payment')
            @endif

...
class Payments2 extends Component
{
    ...
    public $user_without_payment_add;
    ...
    public function makepayment(Request $request)
    {
        ...
        try {
            ...
            if ($request->new_user) {
                $this->user_without_payment_add = UserWithoutPayments::find($request->user_id)->first();
                $this->useradd = User::create([
                    'name'      => $this->user_without_payment_add->name,
                    'email'     => $this->user_without_payment_add->email,
                    'phone'     => $this->user_without_payment_add->phone,
            ...
            if ($payment) {
                ...
                $this->send_mail_to_coach($this->useradd, $request->price_total, $request->total_teams);
            ...
            if ($this->user_without_payment_add) {
                $this->user_without_payment_add->delete();   // Si existe en tabla temporal lo elimina
            }
    ...
    public function goToNextPage_and_create_user_without()
    {
        ...
        if (!$this->user && !$this->user_without_payment) {
            $this->new_user = true;
        ...
        if (!$this->user_without_payment && !$this->user) {
